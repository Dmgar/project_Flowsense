...
export function RouteCard({ route, label, tone }: { route: RouteResponse; label: string; tone: 'static' | 'dynamic' }) {
  const accent =
    tone === 'dynamic' ? 'text-accent-cyan border-accent-cyan/40' : 'text-text-muted border-border-subtle';
  const eta = useCountUp(route.total_estimated_time_s, 600);
  return (
...
export function DispatchForm() {
  ...
  const setAutoFollowNextDispatch = useStore((s) => s.setAutoFollowNextDispatch);
  ...
  const handleDispatch = async (simulate: boolean) => {
    ...
    try {
      ...
      setAutoFollowNextDispatch(true);
  ...
  return (
    <section className="rounded border border-border-subtle bg-bg-tertiary/40 p-4">
      ...
      >
        {isLoading ? 'CALCULANDO…' : '🚨 SIMULAR DESPACHO (FLOWSENSE)'}
      </button>

      <button
        onClick={() => handleDispatch(false)}
        disabled={isLoading}
        className="mt-2 w-full rounded border border-border-subtle bg-bg-tertiary px-3 py-2 font-mono text-xs font-bold text-text-secondary transition-colors hover:border-text-muted hover:text-text-primary disabled:opacity-40"
      >
        RUTA ESTÁTICA (BASELINE)
      </button>
...
export function ActiveRoutePanel() {
  ...
  const savings = useCountUp(activeRoute?.savings_pct ?? 0, 600);
    ...
    <section className="rounded border border-border-subtle p-4">
      ...
      <div className="space-y-2">
        ...
        {Object.values(vehicles).map((v) => {
          const isFollowed = followUnitId === v.vehicle_id;
          return (
            <div
              key={v.vehicle_id}
              onClick={() => setFollowUnitId(isFollowed ? null : v.vehicle_id)}
              className={`flex cursor-pointer items-center justify-between rounded border px-2 py-1.5 transition-colors ${
                isFollowed
                  ? 'border-accent-cyan bg-accent-cyan/10'
                  : 'border-transparent bg-bg-tertiary/60 hover:bg-bg-hover'
              }`}
            >
              <div>
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`h-2 w-2 rounded-full ${
                    v.speed_kmh > 0 ? 'bg-emerald-400 animate-pulse' : 'bg-slate-600'
                  }`}
                />
                <span
                  className={`font-mono text-[9px] ${isFollowed ? 'text-accent-cyan' : 'text-text-muted'}`}
                >
...
export function Sidebar() {
  return (
    <aside className="hidden w-80 flex-col gap-4 overflow-y-auto border-l border-border-primary bg-bg-panel p-4 md:flex lg:w-80 xl:w-80">
      <DispatchForm />
      <ActiveRoutePanel />
      <ActiveUnitsPanel />
    </aside>
